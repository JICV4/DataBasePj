...
def requestAPI(linkAPI):
    ...
    with requests.get(linkAPI,stream=True) as response:
        ...
        if codeget == 200:
            ...
            print("Data recived")
            return textoutput   
        else: #If you made a bad request maybe the data is too old, add it to APIerrors
            return "API was not able to retrive this information"
    #Maybe an error for the try part on the code?

#Parce an example of API (https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/-0.96663%2C-76.93550/2018-01-01/2018-01-02?unitGroup=metric&include=days&key=P2JB9VCV4JPUN4EUHPT6T8L4K&contentType=csv) output to learn how to work with it
...
if len(DBDF) == 0:
    ...
    for i in range(len(DBDF)): #To get the data from the API, limited to 100 request per day
        ...
        try: #Try to get the info
            ...
            except: #If you cant append maybe there is a problem with the data, so drop it for the data base.
    ...
    Workedids = Good + APIerror #If API dont work could be for the limit of queries, need to differenciate from other errors
